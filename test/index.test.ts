...
/** Mocks the feed and the CallMeBot endpoint on a single global fetch. */
function scenario(
  overrides: Partial<Env> = {},
  whatsappStatus: (url: URL, call: number) => number = () => 200,
): Scenario {
  ...
  const fetchMock = vi.fn(async (input: string | URL) => {
    ...
    if (url.href.startsWith(CALLMEBOT_ENDPOINT)) {
      sent.push(url);
      whatsappCall += 1;
      const status = whatsappStatus(url, whatsappCall);
      return new Response("queued", { status });
    }
  ...
  return { kv, env, fetchMock, whatsappCalls: () => sent };
}
...
describe("runNotifier", () => {
  it("notifies every new release and records them in KV", async () => {
    const { env, kv, whatsappCalls } = scenario();
    ...
    expect(whatsappCalls()).toHaveLength(3);
  ...
  it("sends only the releases not seen before", async () => {
    ...
    expect(whatsappCalls()[0]?.searchParams.get("text")).toContain("Cyber Drift 2 & The Lost City");
  });

  it("is idempotent across consecutive runs", async () => {
    const { env, whatsappCalls } = scenario();
  ...
  it("caps the batch at MAX_NOTIFICATIONS_PER_RUN and defers the rest", async () => {
    const { env, kv, whatsappCalls } = scenario({ MAX_NOTIFICATIONS_PER_RUN: "2" });
  ...
  it("does not mark a release as seen when WhatsApp delivery fails", async () => {
    // Every attempt for the second release fails with a non-retryable 400.
    const { env, kv } = scenario({}, (url) =>
      url.searchParams.get("text")?.includes("Cyber Drift") === true ? 400 : 200,
    );
  ...
  it("retries the failed release on the next run", async () => {
    ...
    const { env, kv } = scenario({}, (url) =>
      failFirstBatch && url.searchParams.get("text")?.includes("Cyber Drift") === true ? 400 : 200,
    );
  ...
  it("refuses to run without CallMeBot credentials", async () => {
    const { env, whatsappCalls } = scenario({ CALLMEBOT_API_KEY: "" });
...
describe("dryRun", () => {
  it("reports what would be sent without touching KV or WhatsApp", async () => {
    const { env, kv, whatsappCalls } = scenario({ MAX_NOTIFICATIONS_PER_RUN: "2" });
...
describe("worker handlers", () => {
  ...
  it("GET /test reports feed errors as JSON without throwing", async () => {
    const kv = new MemoryKV();
  ...
  it("scheduled sends only new releases and keeps KV consistent", async () => {
    ...
    await worker.scheduled({ cron: "*/15 * * * *", scheduledTime: Date.now() } as ScheduledController, env);
